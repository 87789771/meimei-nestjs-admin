...
export class LogService {
  ...
  async operLogList(reqOperLogDto: ReqOperLogDto): Promise<PaginatedDto<OperLog>> {
    ...
    const queryBuilder = this.operLogRepository.createQueryBuilder('operLog').where(where)
  ...
  /* 清空操作日志 */
  async cleanOperLog() {
    return this.operLogRepository.createQueryBuilder('operLog').delete().execute()
  }
  ...
  async addLogininfor(request: Request, msg: string, token?: string) {
    ...
    if (token) {
      ...
      await this.redis.set(`${USER_ONLINE_KEY}:${user.userId}`, JSON.stringify(loginUser), 'EX', 60 * 60 * 24)
    }
  ...
  async logininforList(reqLogininforDto: ReqLogininforDto): Promise<PaginatedDto<Logininfor>> {
    ...
    const queryBuilder = this.logininforRepository.createQueryBuilder('logininfor').where(where)
  ...
  async cleanLogininfor() {
    return this.logininforRepository.createQueryBuilder('logininfor').delete().execute()
  }
}
