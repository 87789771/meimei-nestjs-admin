...
export class DictService {
  ...
  async deleteByDictIdArr(dictIdArr: string[]) {
    ...
    if (errorList.length) {
      ...
      throw new ApiException(`字典编号为${idArr.join('、')}的字典存在字典值，请先删除字典值`)
    } else {
  ...
  async getDictDataByDictType(dictType: string): Promise<DictData[]> {
    ...
    if (dictDataArrString) {
      ...
      const dictDataArr = await this.dictDataRepository
        ...
        .innerJoin('dictData.dictType', 'dictType', 'dictType.status = 0 and dictType.dictType = :dictType', {
          dictType,
        })
  ...
  async dictDataList(reqDictDataListDto: ReqDictDataListDto): Promise<PaginatedDto<DictData>> {
    ...
    const result = await this.dictDataRepository
      .createQueryBuilder('dictData')
      .innerJoin('dictData.dictType', 'dictType', 'dictType.dictType = :dictType', {
  ...
  async addOrUpdateDictData(reqAddDictDataDto: ReqAddDictDataDto) {
    ...
    const dictData = Object.assign(new DictData(), reqAddDictDataDto) as DictData
