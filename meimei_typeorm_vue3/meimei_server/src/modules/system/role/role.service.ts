...
export class RoleService {
  ...
  async changeStatus(roleId: number, status: string, updateBy: string) {
    return await this.roleRepository.createQueryBuilder().update().set({ status, updateBy }).where({ roleId }).execute()
  }
  ...
  async cancel(roleId: number, userIdArr: number[] | string[]) {
    return await this.roleRepository.createQueryBuilder('role').relation('users').of(roleId).remove(userIdArr)
  }

  /* 给角色添加用户 */
  async selectAll(roleId: number, userIdArr: number[] | string[]) {
    return await this.roleRepository.createQueryBuilder('role').relation('users').of(roleId).add(userIdArr)
  }
}
