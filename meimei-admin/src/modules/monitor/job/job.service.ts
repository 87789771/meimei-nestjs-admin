...
export class JobService {
    constructor(
        ...
        @InjectRepository(JobLog) private readonly jobLogRepository: Repository<JobLog>,
    ...
    async changeStatus(reqChangStatusDto: ReqChangStatusDto, updateBy: string) {
        await this.jobRepository.createQueryBuilder('job')
            .update()
            .set({
    ...
    /* 清空调度日志 */
    async cleanJobLog() {
        return await this.jobLogRepository.createQueryBuilder('jobLog')
            .delete()
            .execute()
    }
    ...
    /* 启动定时任务 */
    async start(job: Job) {
        let repeat: CronRepeatOptions = {
            cron: job.cronExpression
        }
        const oneJob = await this.jobQueue.add(job,
            {
                jobId: job.jobId,
    ...
    async misfirePolicy(jobArr: Job[]) {
        ...
        jobArr.forEach(job => {
            ...
            if (job.misfirePolicy == '2' && !executeOnce.find(job2 => job.jobId == job2.jobId)) {
    ...
    async analysisinvokeTarget(job: Job) {
        ...
        let splitArr = invokeTarget.split('.')
        if (splitArr.length != 2) throw new ApiException('调用方法格式错误')
        const serviceName = splitArr[0]
        if (!(splitArr[1].includes('(') && splitArr[1].includes(')'))) throw new ApiException('调用方法格式错误')
